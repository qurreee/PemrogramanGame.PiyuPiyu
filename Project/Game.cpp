...
void Game::Run()
{
	...
	Init(); //method user-defined, custom
	GameLoop(window);
	SDL_Quit();
}
...
////addons
//float Engine::Game::GetGameTime(deltatime deltatime)
//{
//
//}
...
SDL_Window* Game::InitInternal()
{
	...
	if (setting->targetFrameRate > 0) {
		targetFrameTime = 1000.0f / setting->targetFrameRate; //limit framerate
	}
	...
	SDL_Window* window = SDL_CreateWindow(setting->windowTitle.c_str(),
		SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, //lokasi windows
	...
	// Init Audio
	int audioFlags = MIX_INIT_MP3 | MIX_INIT_FLAC | MIX_INIT_OGG; //jangan pake mp3, soalnya kudu bayar, pakai OGG, FLAC untuk suara lebih bagus tapi file lebih besar
